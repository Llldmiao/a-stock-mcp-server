    ...
    async def get_stock_history(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if period == "daily":
                ...
                hist_data = ak.stock_zh_a_hist(symbol=symbol, period="weekly",
                                              start_date=start_date, end_date=end_date)
            elif period == "monthly":
                hist_data = ak.stock_zh_a_hist(symbol=symbol, period="monthly",
                                              start_date=start_date, end_date=end_date)
            else:
            ...
            for _, row in recent_data.iterrows():
                result += f"{row['日期']}\t{row['开盘']}\t{row['收盘']}\t{row['最高']}\t{row['最低']}\t{row['成交量']:,}\t{row['成交额']:,}\n"
